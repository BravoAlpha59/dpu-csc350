...
void drawSphere (int numSlices, int numStacks)
{
  ...
  Debug("Stacks:" << numStacks << " Slices:" << numSlices << endl);
  Debug("deltaTheta:" << deltaTheta << " radians " << deltaTheta * (180 / M_PI) << " degrees" << endl);
  Debug("deltaY:" << deltaY  << endl);
  Debug("----------" << endl);
  ...
  Debug(numStacks << ": y=" << y[numStacks] << " r=" << r[numStacks] << endl);
  Debug("++++++++++" << endl);
  ...
  for(j = 0; j <= numSlices; j++) {
      ...
      // close the top
      glNormal3d(leftX[0] , y[0], leftZ[0]);
      glVertex3d(leftX[0] , y[0], leftZ[0]);
      glNormal3d(rightX[0] , y[0], rightZ[0]);
      glVertex3d(rightX[0] , y[0], rightZ[0]);
      ...
      glNormal3d(rightX[i] , y[i], rightZ[i]);
      glVertex3d(rightX[i] , y[i], rightZ[i]);
      glNormal3d(rightX[i+1], -1, rightZ[i+1]);
      glVertex3d(rightX[i+1], -1, rightZ[i+1]);
      glNormal3d(leftX[i+1] , -1, leftZ[i+1]);
      glVertex3d(leftX[i+1] , -1, leftZ[i+1]);
